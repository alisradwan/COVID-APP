let map;
let centerUSLatLong = { lat: 38.5, lng: -98.0 };
...
searchform.addEventListener("submit", function search(e) {
  ...
  fetch(
    "https://covid-19-statistics.p.rapidapi.com/reports?q=" + userInput,
    options
  )
...
function SafeUserInput() {
  ...
  var Usercityinput = JSON.parse(localStorage.getItem("citys")) || [];
  Usercityinput.push(UserInputt);
  localStorage.setItem("citys", JSON.stringify(Usercityinput));
}

search.onclick = SafeUserInput;
